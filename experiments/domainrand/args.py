...
def get_args():
    ...
    lunar_subparser.add_argument("--randomized-env-id", default="LunarLanderRandomized-v0", 
    ...
    pusher_subparser.add_argument("--randomized-env-id", default="Pusher3DOFRandomized-v0",
        type=str, help="Name of the reference environment")
    ...
    ergo_subparser.add_argument("--randomized-env-id", default="ErgoReacher4DOFRandomizedEasy-v0", 
        type=str, help="Name of the reference environment")
    ...
    for subparser in [lunar_subparser, pusher_subparser, ergo_subparser, backlash_subparser, ergosix_subparser]:
        subparser.add_argument("--experiment-name", type=str, default='unfreeze-policy',
        ...
        subparser.add_argument("--svpg-rollout-length", default=10, type=int, 
        ...
        subparser.add_argument("--initial-svpg-steps", default=0, type=int, 
            help="number of svpg steps to take before updates")
        subparser.add_argument("--max-agent-timesteps", default=2e6, type=int, 
            help="max iterations, counted in terms of AGENT env steps")
        subparser.add_argument("--episodes-per-instance", default=1, type=int, 
            help="number of episodes to rollout the agent for per sim instance") # 每个env param的环境跑几个episodes
        ...
        subparser.add_argument('--sampling_gamma_0', type=float, default=1.0)
        subparser.add_argument('--sampling_gamma_1', type=float, default=5.0)
        subparser.add_argument('--sampling_gamma_2', type=float, default=8.0)
        subparser.add_argument('--cvar', default=0.5, type=float)
        subparser.add_argument('--gdroweight', default=0.01, type=float)
        subparser.add_argument('--no_add_random', default=False, action='store_true')
        subparser.add_argument('--no_batch_norm', default=False, action='store_true')
        subparser.add_argument('--sampler_lr', default=0.005, type=float)
        subparser.add_argument('--sampler_train_times', default=10, type=int)
        subparser.add_argument('--sampler_multiplier', default=2.5, type=float)
        subparser.add_argument('--kl_weight', type=float, default=1.0)
        subparser.add_argument('--output_type', type=str, default="deterministic")#"probabilistic"
        subparser.add_argument('--uniform_sample_steps', default=0.1, type=float)
        subparser.add_argument('--folder', type=str, default="train")
        subparser.add_argument('--wandb_project', type=str, default="dr")
        subparser.add_argument('--use_dist', default=False, action='store_true')
        subparser.add_argument('--algo', type=str, default="erm")#"erm", "drm", "gdrm", "mpts","diverse_mpts"
        subparser.add_argument('--posterior_sampling', default=False, action='store_true')
        subparser.add_argument('--L2', default=False, action='store_true')
