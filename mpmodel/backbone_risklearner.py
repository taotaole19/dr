...
class Decoder_me_tnpd(nn.Module):
    ...
    def forward(self, x, z, output_type):
        # batch_size=1
        # z: batch_size, num_tar, d_model
        # x: batch_size, num_tar, x_dim
        batch_size, num_tar, _ = x.size()
        _, _, d_model = z.size()
        # +++++++++acqusition_function+++++++++++
        num_samples = 30
        all_samples = []
        ...
        if batch_size == 1:
            ...
            input_pairs = torch.cat((x, z), dim=-1)
            ...
            if output_type == "deterministic":
                ...
                p_y_pred_dis = Normal(mu, sigma)
                return mu, sigma
            elif output_type == "probabilistic":
                sigma = 0.1 + 0.9 * F.softplus(pre_sigma)
                p_y_pred_dis = Normal(mu, sigma)
                p_y_pred = p_y_pred_dis
            # [batch_size, num_tar, y_dim]
                return p_y_pred
